...
enum GAME_STATE{NEW_GAME, SELECTION, PLACEMENT, BATTLE, RESULTS}
...
func _bey_death(point_type : POINT_TYPE, point_winner_id : int = -1):
	if point_type == POINT_TYPE.DESTRUCTION: _on_bey_burst()
...
func _on_bey_burst():
	...
	tween.tween_property(Engine,"time_scale", 0.3, 0.2)
